...
3. **一套课件一个世界**：所有页走 `theme.js` 母版，**一份课件一张背景**（`assets.bg`），字体 ≤2，**用系统自带的微软雅黑（典礼标题可用宋体）**，不随包附 `字体/`。调色板 ≤6，插画是套件（封面 + 背景 + 4:3 小图 + 角色剪影），不是每页一张画。合集每节点一个主题，风格从用户参考图提取。
4. **密度**：正文页 ≥3 信息单元、每单元 3–4 行完整句、≥60 字、一句横幅。`qa-density.py` FAIL 不许打包。版式 ≥6 种，卡片页 ≤35%。
5. 页表、单元、讲稿栏目、实物件字段，禁止从上一份改词接着用。
6. 实物件少而准：正好 1 个钩子 + 最多 1 个当场表，文件夹 ≤3 个文件。PPT 上已有的表不要再做打印件。班名 / 姓名 / 日期 / 岗位是可编辑文本框。见 [content.md](content.md)。
...
某一边看着对不代表另一边对：字体用微软雅黑（Windows / WPS 自带），行距 / 段距显式写死，末段不留空行，项目符号缩进固定。这些 `theme.js` 已内置，不要绕过它手写文本框。不随包塞 otf。
...
- [ ] 字体 ≤2，微软雅黑（典礼可用宋体），客户包里没有 `字体/`
- [ ] 实物件 = 1 钩子 + 至多 1 当场表，≤3 个文件；可编辑处是文本框
